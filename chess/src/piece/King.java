    ...
    private boolean isValidCastling(Board board, Box start, Box end) {
        ...
        if (this.isWhite()
            ...
            if (Math.abs(end.getY() - start.getY()) == 2) {
                // check if there the piece.Rook is in the correct position
                // check if there is no piece between piece.Rook and the piece.King
                // check if the piece.King or the piece.Rook has not moved before
